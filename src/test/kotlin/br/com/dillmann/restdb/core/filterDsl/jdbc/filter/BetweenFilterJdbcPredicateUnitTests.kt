 ...
 */
class BetweenFilterJdbcPredicateUnitTests {

    private val leftValue = randomString()
    private val rightValue = randomString()
    private val columnName = randomString()
    private val predicate = BetweenFilterJdbcPredicate(columnName, leftValue, rightValue)

    @Test
    fun `output sql should use between instruction`() {
        // execution
        val sql = predicate.sqlInstructions

        // validation
        assertTrue { sql.contains("BETWEEN", ignoreCase = true) }
    }
