 ...
 */
class AndLogicalOperatorJdbcPredicateUnitTests {

    private val leftPredicate: JdbcPredicate = mockk()
    private val rightPredicate: JdbcPredicate = mockk()
    private lateinit var predicate: AndLogicalOperatorJdbcPredicate
    ...
    @Test
    fun `sql instructions should contain left and right predicates instructions joined using an AND`() {
        // scenario
        val leftSql = randomString()
        val rightSql = randomString()
        val expectedResult = "$leftSql AND $rightSql"
        every { leftPredicate.sqlInstructions } returns leftSql
        every { rightPredicate.sqlInstructions } returns rightSql
        predicate = AndLogicalOperatorJdbcPredicate(leftPredicate, rightPredicate)
